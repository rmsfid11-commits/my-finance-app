...
function HouseholdTab({ profile, goals, budget, setBudget, transactions, fixedExpenses, setFixedExpenses, addTransaction, deleteTransaction }) {
  ...
  return (
    <div className="animate-slide">
      ...
      {subTab === 'quick' && <QuickInput addTransaction={addTransaction} />}
      {subTab === 'daily' && <DailyView transactions={transactions} budget={budget} deleteTransaction={deleteTransaction} />}
      {subTab === 'weekly' && <WeeklyView transactions={transactions} budget={budget} />}
      {subTab === 'monthly' && <MonthlyView transactions={transactions} budget={budget} setBudget={setBudget} profile={profile} fixedExpenses={fixedExpenses} />}
      {subTab === 'fixed' && <FixedView fixedExpenses={fixedExpenses} setFixedExpenses={setFixedExpenses} />}
      {subTab === 'challenge' && <ChallengeView transactions={transactions} budget={budget} />}
      {subTab === 'pattern' && <PatternView transactions={transactions} />}
    </div>
  );
}

function QuickInput({ addTransaction }) {
    ...
    <div className="space-y-4">
      <div className="bg-c-card border border-c-border rounded-lg p-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          {QUICK_INPUTS.map(item => (<button key={item.label} onClick={() => handleQuick(item)} className="bg-c-bg hover:bg-c-subtle border border-c-border rounded-lg p-4 text-center transition-all active:scale-95"><div className="text-sm font-semibold text-c-text mb-1">{item.label}</div><div className="text-sm text-c-text2">{formatFullKRW(item.amount)}</div></button>))}
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3 mt-5">
        <button onClick={() => setShowManual(!showManual)} className="bg-c-card border border-c-border rounded-lg p-6 flex items-center gap-3 text-base font-semibold text-c-text transition-colors hover:border-[#3182F6]/50 active:scale-[0.98]"><Plus size={22} className="text-[#3182F6]" /> 직접 입력</button>
        <button onClick={() => setShowSMS(!showSMS)} className="bg-c-card border border-c-border rounded-lg p-6 flex items-center gap-3 text-base font-semibold text-c-text transition-colors hover:border-[#3182F6]/50 active:scale-[0.98]"><MessageSquare size={22} className="text-green-500" /> SMS 입력</button>
        <button className="bg-c-card border border-c-border rounded-lg p-6 flex items-center gap-3 text-base font-semibold text-c-text opacity-50"><Camera size={22} className="text-purple-500" /> 영수증 OCR</button>
        <button className="bg-c-card border border-c-border rounded-lg p-6 flex items-center gap-3 text-base font-semibold text-c-text opacity-50"><Mic size={22} className="text-orange-500" /> 음성 입력</button>
      </div>
      {showManual && <div className="bg-c-card border border-c-border rounded-lg p-5 space-y-3 animate-fade"><h3 className="font-bold text-base text-c-text">직접 입력</h3><div><label className="text-xs text-c-text2">금액</label><input type="number" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} placeholder="금액 입력" /></div><div><label className="text-xs text-c-text2">카테고리</label><select value={form.category} onChange={e => setForm({...form, category: e.target.value})}>{CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select></div><div><label className="text-xs text-c-text2">장소</label><input type="text" value={form.place} onChange={e => setForm({...form, place: e.target.value})} placeholder="장소 (선택)" /></div><div><label className="text-xs text-c-text2">메모</label><input type="text" value={form.memo} onChange={e => setForm({...form, memo: e.target.value})} placeholder="메모 (선택)" /></div><button onClick={handleManual} className="w-full btn-primary py-3">저장하기</button></div>}
      {showSMS && <div className="bg-c-card border border-c-border rounded-lg p-5 space-y-3 animate-fade"><h3 className="font-bold text-base text-c-text">SMS 자동인식</h3><textarea value={smsText} onChange={e => setSmsText(e.target.value)} placeholder={"카드 사용 문자를 붙여넣기 하세요\n예: [신한] 15,000원 승인 스타벅스"} rows={3} className="w-full border border-c-border bg-c-bg text-c-text rounded-xl p-3 text-sm" /><button onClick={handleSMS} className="w-full btn-primary py-3">인식하기</button></div>}
    </div>
  );
}

function DailyView({ transactions, budget, deleteTransaction }) {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-c-card border border-c-border rounded-lg p-5">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <div className="text-center bg-c-bg border border-c-border rounded-lg p-4"><div className="text-sm font-medium text-c-text2 mb-1">총 지출</div><div className="text-base font-bold text-red-500">{formatFullKRW(dayTotal)}</div></div>
          <div className="text-center bg-c-bg border border-c-border rounded-lg p-4"><div className="text-sm font-medium text-c-text2 mb-1">일 예산</div><div className="text-base font-bold text-c-text">{formatFullKRW(dailyBudget)}</div></div>
          <div className={`text-center bg-c-bg border border-c-border rounded-lg p-4`}><div className="text-sm font-medium text-c-text2 mb-1">잔여</div><div className={`text-base font-bold ${dailyBudget-dayTotal>=0?'text-green-500':'text-red-500'}`}>{formatFullKRW(dailyBudget-dayTotal)}</div></div>
        </div>
    ...
    <div className="space-y-4">
      <div className="bg-c-card border border-c-border rounded-lg p-5">
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          <div className="bg-c-bg border border-c-border rounded-lg p-4 text-center"><div className="text-sm font-medium text-red-500 mb-1">주간 지출</div><div className="text-xl font-bold text-red-500">{formatKRW(weekTotal)}</div></div>
          <div className="bg-c-bg border border-c-border rounded-lg p-4 text-center"><div className="text-sm font-medium text-c-text2 mb-1">일 평균</div><div className="text-xl font-bold text-c-text">{formatFullKRW(Math.round(weekTotal/7))}</div></div>
        </div>
      ...
      <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-5 text-white">
        ...
        <div className="flex justify-between text-sm"><span>내 주간 지출</span><span className="font-bold">{formatKRW(weekTotal)}</span></div>
        <div className="flex justify-between text-sm"><span>또래 평균</span><span className="font-semibold">{formatKRW(peerAvg)}</span></div>
        <div className={`text-xs mt-2 font-semibold ${weekTotal<peerAvg?'text-green-300':'text-red-300'}`}>{weekTotal<peerAvg?`또래보다 ${formatFullKRW(peerAvg-weekTotal)} 적게 쓰고 있어요!`:`또래보다 ${formatFullKRW(weekTotal-peerAvg)} 더 쓰고 있어요`}</div>
      </div>
    </div>
  );
}

function MonthlyView({ transactions, budget, setBudget, profile, fixedExpenses }) {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-c-card border border-c-border rounded-lg p-5">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <div className="bg-c-bg border border-c-border rounded-lg p-4 text-center"><div className="text-sm font-medium text-green-500 mb-1">수입</div><div className="text-base font-bold text-green-500">{formatKRW(profile.salary)}</div></div>
          <div className="bg-c-bg border border-c-border rounded-lg p-4 text-center"><div className="text-sm font-medium text-red-500 mb-1">지출</div><div className="text-base font-bold text-red-500">{formatKRW(totalExpense)}</div></div>
          <div className="bg-c-bg border border-c-border rounded-lg p-4 text-center"><div className="text-sm font-medium text-purple-500 mb-1">저축</div><div className="text-base font-bold text-purple-500">{formatKRW(savings)}</div></div>
        </div>
        <div className="bg-c-bg border border-c-border rounded-lg p-4"><div className="flex justify-between text-xs mb-2"><span className="font-medium text-c-text">저축률</span><span className="font-bold text-purple-500">{savingRate}%</span></div><div className="progress-bar"><div className="progress-fill bg-purple-500" style={{width:`${Math.min(Math.max(parseFloat(savingRate),0),100)}%`}} /></div></div>
      </div>
      <div className="bg-c-card border border-c-border rounded-lg p-5">
        ...
        <div className="space-y-3">{catBreakdown.map(c=><div key={c.name} className="border-b border-c-border pb-3"><div className="flex justify-between items-center mb-2"><span className="text-base font-medium text-c-text">{c.name}</span><span className="text-base font-bold text-c-text">{formatFullKRW(c.value)}</span></div>{c.budget>0&&<><div className="progress-bar"><div className={`progress-fill ${parseInt(c.usage)>100?'bg-red-500':parseInt(c.usage)>80?'bg-yellow-500':'bg-green-500'}`} style={{width:`${Math.min(parseInt(c.usage)||0,100)}%`}} /></div><div className="flex justify-between text-sm text-c-text2 mt-1.5"><span>예산 <EditableNumber value={c.budget} onSave={(v) => setBudget(prev => ({...prev, [c.name]: Math.round(v)}))} format={formatFullKRW} /></span><span className={parseInt(c.usage)>100?'text-red-500 font-bold':''}>{c.usage}%</span></div></>}</div>)}</div>
      </div>
      <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-5 text-white">
        <h3 className="font-bold text-sm mb-3">또래 비교 (35세 간호사 600명)</h3>
        <div className="grid grid-cols-2 gap-3"><div className="bg-white/20 rounded-lg p-3 text-center"><div className="text-xs opacity-80">내 저축률</div><div className="text-xl font-bold">{savingRate}%</div></div><div className="bg-white/20 rounded-lg p-3 text-center"><div className="text-xs opacity-80">또래 평균</div><div className="text-xl font-bold">{peerStats.avgRate}%</div></div></div>
      ...
      <div className="bg-c-card border border-c-border rounded-lg p-5">
        ...
        <div className="space-y-2 text-sm">
          <div className="bg-c-bg border border-c-border rounded-lg p-4"><div className="font-bold text-green-500 mb-1">강점</div><p className="text-green-400 text-xs">{parseFloat(savingRate)>=30?'저축률이 좋습니다! 꾸준히 유지하세요.':'가계부 기록을 시작한 것 자체가 훌륭합니다!'}</p></div>
          <div className="bg-c-bg border border-c-border rounded-lg p-4"><div className="font-bold text-yellow-500 mb-1">개선점</div><p className="text-yellow-400 text-xs">도시락이나 집밥을 활용하면 식비를 줄일 수 있어요.</p></div>
          <div className="bg-c-bg border border-c-border rounded-lg p-4"><div className="font-bold text-blue-500 mb-1">다음달 목표</div><p className="text-blue-400 text-xs">저축률 {Math.min(parseFloat(savingRate)+5,50)}%를 목표로 해보세요!</p></div>
        </div>
      </div>
    </div>
  );
...
function FixedView({ fixedExpenses, setFixedExpenses }) {
    ...
    <div className="space-y-4">
      <div className="bg-c-card border border-c-border rounded-lg p-5">
        ...
        <div className="space-y-3">{challenges.filter(c=>c.active).map(c=>{
          ...
          return (<div key={c.id} className="bg-c-bg border border-c-border rounded-lg p-5"><div className="flex justify-between mb-2.5"><span className="text-base font-semibold text-c-text">{c.name}</span><span className={`text-sm font-bold ${isOk?'text-green-500':'text-orange-500'}`}>{c.type==='count'?`${c.current}/${c.target}일`:`${formatFullKRW(c.current)} / ${formatFullKRW(c.target)}`}</span></div><div className="progress-bar"><div className={`progress-fill ${isOk?'bg-green-500':'bg-orange-500'}`} style={{width:`${Math.min(pct,100)}%`}} /></div></div>);
        })}</div>
      </div>
      <div className="bg-c-card border border-c-border rounded-lg p-5"><h3 className="font-bold text-base text-c-text mb-4">추천 챌린지</h3><div className="space-y-2.5">{['커피 비용 3만원 이하','한달 저축률 40%','1주일 외식 제로'].map((c,i)=><div key={i} className="flex items-center justify-between bg-c-bg border border-c-border rounded-lg p-5"><span className="text-base font-medium text-c-text">{c}</span><button className="text-sm bg-[#3182F6] text-white px-5 py-2 rounded-xl font-semibold">참여</button></div>)}</div></div>
      <div className="bg-c-bg border border-c-border rounded-lg p-5"><h3 className="font-bold text-sm mb-2 text-green-500">보상 시스템</h3><p className="text-xs text-green-400">챌린지 3개 달성 시 절약액의 10%를 자유롭게 사용하세요!</p></div>
    </div>
  );
}

function PatternView({ transactions }) {
  const currentMonth = new Date().toISOString().substring(0,7);
  const monthTx = useMemo(()=>transactions.filter(t=>t.date.startsWith(currentMonth)),[transactions,currentMonth]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-c-card border border-c-border rounded-lg p-5"><h3 className="font-bold text-base text-c-text mb-4">결제 수단별</h3><div className="space-y-2.5">{paymentData.sort((a,b)=>b.value-a.value).map(p=><div key={p.name} className="flex items-center gap-2"><span className="text-xs w-20 text-c-text2 font-medium">{p.name}</span><div className="flex-1 h-6 bg-c-subtle rounded-full overflow-hidden"><div className="h-full bg-[#3182F6] rounded-full transition-all duration-500" style={{width:`${(p.value/Math.max(...paymentData.map(x=>x.value)))*100}%`}} /></div><span className="text-xs font-bold text-c-text w-20 text-right">{formatKRW(p.value)}</span></div>)}</div></div>
      <div className="bg-c-card border border-c-border rounded-lg p-5"><h3 className="font-bold text-base text-c-text mb-4">자주 가는 곳 TOP 5</h3><div className="space-y-2">{topPlaces.map((p,i)=><div key={p.name} className="flex items-center gap-3 py-2"><span className="w-7 h-7 rounded-full bg-[#3182F6] text-white text-xs flex items-center justify-center font-bold">{i+1}</span><div className="flex-1"><div className="text-sm font-semibold text-c-text">{p.name}</div><div className="text-xs text-c-text2">{p.count}회 방문</div></div><span className="text-sm font-bold text-c-text">{formatFullKRW(p.amount)}</span></div>)}{topPlaces.length===0&&<div className="text-sm text-c-text3 text-center py-6">데이터가 부족합니다</div>}</div></div>
      <div className="bg-c-bg border border-c-border rounded-lg p-5"><h3 className="font-bold text-sm mb-2 text-blue-500">AI 인사이트</h3><div className="text-xs text-blue-400 space-y-1.5"><p>• 점심시간(12-14시)에 가장 많이 지출하고 있어요.</p><p>• 주말 지출이 평일보다 높은 편이에요.</p><p>• 도시락이나 집밥을 활용하면 식비를 30% 줄일 수 있어요.</p></div></div>
    </div>
  );
}
